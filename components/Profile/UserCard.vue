<template>
  <v-card :style="{background: $vuetify.theme.themes.dark.background2}" class="bg">
    <v-row class="bg">
      <v-col cols="12" align="center" justify="end">
        <div class="block block-1" />
        <div class="block block-2" />
        <v-avatar size="100">
          <v-img :src="this.$store.state.profile.avatar" />
        </v-avatar>
      </v-col>
    </v-row>
    <v-row class="mt-9">
      <v-col>
        ...
        <v-card-text>{{ this.$store.state.profile.post }}</v-card-text>
